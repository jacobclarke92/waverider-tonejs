...
function addUrlBehavior(tableDB, blobKey) {
	...
	tableDB.prototype.readAsText = function() {
		return readBlobAsText(this[blobKey])
	}
	tableDB.prototype.readAsArrayBuffer = function() {
		return readBlobAsArrayBuffer(this[blobKey])
	}
...
export const getBy = (table, field, value) =>
...
export const getWhere = (table, conditions = {}) =>
	db
		.table(table)
		.where(conditions)
		.toArray()

export const getFirstWhere = (table, conditions = {}) =>
	db
		.table(table)
		.where(conditions)
		.first()

export const getList = (table, fieldPath) =>
	db
		.table(table)
		.toArray()
		.map(item => _get(item, fieldPath))

export const updateBy = (table, field, value, updates = {}) =>
	new Promise((resolve, reject) =>
		db
			.table(table)
			.where(field)
...
export const updateById = (table, id, updates = {}) =>
	new Promise((resolve, reject) =>
		db
			.table(table)
			.update(id, updates)
...
export const bulkPut = (table, entities) =>
	new Promise((resolve, reject) =>
		db
			.table(table)
			.bulkPut(entities)
			.then(lastKey => resolve(entities))
			.catch(/*BulkError/*/ err, function(e) {
