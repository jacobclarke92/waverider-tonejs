...
export const loadDevices = () => dispatch => getAll('devices')
	.then(devices => dispatch({type: DEVICES_UPDATED, devices}))
	.catch(e => console.warn('Unable to load previous midi devices', e))

export const updateDevices = devicesToUpdate => dispatch => bulkPut('devices', devicesToUpdate)
	.then(updatedDevices => getAll('devices')
		.then(devices => dispatch({type: DEVICES_UPDATED, devices}))
